...
public:
    ...
    DataCost(CostVal *cost) {
        m_costArray = cost;
        m_type = MRF::ARRAY;
    };

    DataCost(DataCostFn costFn) {
        m_costFn = costFn;
        m_type = MRF::FUNCTION;
    };
private:
...
// size nLabels*nLabels, use constructor SmoothnessCost(V). 
// If in addition, there are 
// are spatially varying weights use constructor SmoothnessCost(V,hWeights,vWeights)
...
public:
    ...
    // In case if used for 2D grids, the smoothness term WILL NOT be spatially varying

    SmoothnessCost(int smoothExp, CostVal smoothMax, CostVal lambda) {
        m_type = MRF::THREE_PARAM;
    ...
    // the smoothness term WILL BE be spatially varying
    ...
    SmoothnessCost(MRF::SmoothCostGeneralFn costFn) {
        m_costFn = costFn;
        m_type = MRF::FUNCTION;
        m_varWeights = false;
    };

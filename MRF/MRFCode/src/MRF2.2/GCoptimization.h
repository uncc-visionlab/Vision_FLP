...
public:
    ...
    inline LabelType getLabel(PixelType pixel) {
        assert(pixel >= 0 && pixel < m_nPixels);
        return (m_labeling[pixel]);
    };
    ...
    inline void setLabel(PixelType pixel, LabelType label) {
        assert(label >= 0 && label < m_nLabels && pixel >= 0 && pixel < m_nPixels);
        m_labeling[pixel] = label;
    };
